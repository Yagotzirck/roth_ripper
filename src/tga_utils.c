...
void rgb6bppToTgaPal(const rgb_palette_t *rgb_palette){
    ...
    for(i = 1; i < 256; ++i){
        ...
        */
        tga_palette[i].blue = (rgb_palette[i].blue << 2) | (rgb_palette[i].blue >> 4);
        tga_palette[i].green = (rgb_palette[i].green << 2) | (rgb_palette[i].green >> 4);
        tga_palette[i].red = (rgb_palette[i].red << 2) | (rgb_palette[i].red >> 4);
...
void rgb8bppToTgaPal(const rgb_palette_t *rgb_palette){
    ...
    for(i = 1; i < 256; ++i){
        ...
        tga_palette[i].blue = rgb_palette[i].blue;
        tga_palette[i].green = rgb_palette[i].green;
        tga_palette[i].red = rgb_palette[i].red;
        tga_palette[i].alpha = 0xFF;
    }
}
